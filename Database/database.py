...
def connect(mongo_uri, database, collection_name):
    ...
    with console.status("[bright_magenta]Connecting to DataBase...") as status:
        try:
            ...
            console.log(
                f"[bright_red]Error occured while fetching Client[/bright_red]❌")
        ...
        try:
            ...
            console.log(
                f"[bright_red]Error occured while fetching Database[/bright_red]❌")
            console.log(f"Error: {e}")
        ...
        try:
            ...
            console.log(
                f"[bright_red]Error occured while fetching Collection[/bright_red]❌")
            console.log(f"Error: {e}")
...
def setup(collection, expire_days: int, inactive_days: int) -> None:
    ...
    try:
        ...
        console.log(
            f"[bright_green]Successfully created expireAt index[/bright_green]✅")
    except Exception as e:
        console.log(
            f"[bright_red]Error occured while creating expireAt index[/bright_red]❌")
        console.log(f"Error: {e}")
    ...
    try:
        ...
        console.log(
            f"[bright_green]Successfully created expireInactiveAt index[/bright_green]✅")
    except Exception as e:
        console.log(
            f"[bright_red]Error occured while creating expireInactiveAt index[/bright_red]❌")
        console.log(f"Error: {e}")
...
def update_view(collection, chotu_url, value=1, extend_date=datetime.utcnow()):
    ...
    if exists(collection, chotu_url):
        ...
        try:
            ...
            console.log(
                f"[bright_green]Successfully updated view and expireInactiveAt for `{chotu_url}[/bright_green]✅")
        except Exception as e:
            console.log(
                f"[bright_red]Error occured while updating view or expireInactiveAt for `{chotu_url}`[/bright_red]❌")
